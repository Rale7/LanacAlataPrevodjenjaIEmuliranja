...
void dodaj_obracanje_literal(BazenLiterala *bl, int literal, int lokacija) {
  for (UlazLiteral *trenutni = bl->prviLiteral; trenutni;
...
void dodaj_obracanje_simbol(BazenLiterala *bl, Simbol *sim, int lokacija) {
  for (UlazSimbol *trenutni = bl->prviSimbol; trenutni;
       trenutni = trenutni->sledeci) {
...
static void razresi_pomeraje_za_literale(Obracanje *prvi, int trenutna_lokacija,
                                         Sekcija *sekcija) {
  for (Obracanje *trenutni = prvi; trenutni; trenutni = trenutni->sledeci) {
    int pomeraj = trenutna_lokacija - (trenutni->lokacija + 4);

    char prva_vrednost =
        *dohvati_sadrzaj(sekcija->sadrzaj, trenutni->lokacija + 2);
    prva_vrednost = (char)(prva_vrednost & 0xF0) | ((pomeraj & 0xF00) >> 8);
    postavi_sadrzaj(sekcija->sadrzaj, trenutni->lokacija + 2, &prva_vrednost,
                    1);

    char druga_vrednost = (char)(pomeraj & 0xFF);
    postavi_sadrzaj(sekcija->sadrzaj, trenutni->lokacija + 3, &druga_vrednost,
                    1);
  }
}

static int razresi_pomeraje_za_simbole(UlazSimbol *us, int trenutna_lokacija,
                                       Sekcija *sekcija) {
  int popunio = 0;

  for (Obracanje *trenutni = us->prvi; trenutni; trenutni = trenutni->sledeci) {
    if (us->simbol->tvf->nrz(us->simbol, sekcija, trenutna_lokacija,
                             trenutni->lokacija)) {
      popunio = 1;
    }
  }
...
static void upisivanje_literala(BazenLiterala *bl, Sekcija *sekcija,
                                UlazLiteral *trenutni_literal) {
  postavi_sadrzaj(sekcija->sadrzaj, sekcija->location_counter,
                  (const char *)&trenutni_literal->vrednost, 4);
  razresi_pomeraje_za_literale(trenutni_literal->prvi,
                               sekcija->location_counter, sekcija);
  trenutni_literal->lokacija = sekcija->location_counter;
  sekcija->location_counter += 4;
}

static void upisivanje_simbola(BazenLiterala *bl, Sekcija *sekcija,
                               UlazSimbol *trenutni_simbol) {
  razresi_pomeraje_za_simbole(trenutni_simbol, sekcija->location_counter,
                              sekcija);
  postavi_sadrzaj(sekcija->sadrzaj, sekcija->location_counter,
                  (const char *)&trenutni_simbol->simbol->vrednost, 4);
...
void dodaj_jmp_instrukciju(int lokacija_inst, int lokacija_skoka,
                           SadrzajSekcije *ss) {
  char oc = 0x30;
...
void upisi_bazen(BazenLiterala *bl, int kraj_sekcije) {
  ...
  if (!kraj_sekcije) {
    dodaj_jmp_instrukciju(stari_lc, sekcija->location_counter,
                          sekcija->sadrzaj);
  }
}