...
void bezadresna_inst(int kod_operacije) {
  ...
  postavi_sadrzaj(trenutna->sadrzaj, trenutna->location_counter,
                  (const char *)&vrednost, 1);
  trenutna->location_counter += 1;
  popuni_nulama(trenutna->sadrzaj, trenutna->location_counter, 3);
  trenutna->location_counter += 3;
}

void instrukcija_sa_pomerajem(char kod_operacije, enum Registar ra,
                              enum Registar rb, enum Registar rc, int pomeraj) {
  Sekcija *trenutna_sekcija = dohvati_asembler()->trenutna_sekcija;

  postavi_sadrzaj(trenutna_sekcija->sadrzaj, trenutna_sekcija->location_counter,
                  &kod_operacije, 1);
...
void instrukcija_sa_literalom_bazen(char oc, enum Registar ra, enum Registar rb,
                                    enum Registar rc, int literal) {
  Sekcija *trenutna_sekcija = dohvati_asembler()->trenutna_sekcija;
  int lokacija_literala;

  if (trenutna_sekcija->prethodni_bl &&
      provera_postoji_literal(trenutna_sekcija->prethodni_bl, literal,
                              &lokacija_literala)) {
  ...
  dodaj_obracanje_literal(trenutna_sekcija->bazen_literala, literal,
                          trenutna_sekcija->location_counter);
  instrukcija_sa_pomerajem(oc, ra, rb, rc, 0);
}

void instrukcija_sa_simbol_bazen(char oc, enum Registar ra, enum Registar rb,
                                 enum Registar rc, Simbol *simbol) {
  Sekcija *trenutna_sekcija = dohvati_asembler()->trenutna_sekcija;
  int lokacija_literala;

  if (trenutna_sekcija->prethodni_bl &&
      provera_postoji_simbol(trenutna_sekcija->prethodni_bl, simbol,
                             &lokacija_literala)) {
    int pomeraj = lokacija_literala - (trenutna_sekcija->location_counter + 4);
  ...
  dodaj_obracanje_simbol(trenutna_sekcija->bazen_literala, simbol,
                         trenutna_sekcija->location_counter);
  instrukcija_sa_pomerajem(oc, ra, rb, rc, 0);
}

void skok_literal(int kod_operacije, enum Registar rb, enum Registar rc,
                  int literal) {
...
void skok_simbol(int kod_operacije, enum Registar rb, enum Registar rc,
...
void ld_reg_simbol(enum Registar r1, enum Registar r2,
                   const char *naziv_simbola) {
  TabelaSimbola *ts = dohvati_asembler()->tabel_simbola;
  Simbol *simbol;
  Sekcija *sekcija = dohvati_asembler()->trenutna_sekcija;
...
void st_reg_simbol(enum Registar r1, enum Registar r2,
                   const char *naziv_simbola) {
  TabelaSimbola *ts = dohvati_asembler()->tabel_simbola;
  Simbol *simbol;
  Sekcija *sekcija = dohvati_asembler()->trenutna_sekcija;
