...
static int sab_oduz_rp() { return 2; }

static int mno_delj_rp() { return 3; }

static int otvorena_zagrada_ipr() { return 6; }

static int otvorena_zagrada_spr() { return 0; }

static int zatvorena_zagrada_ipr() { return 1; }

static int zatvorena_zagrada_spr() { return 0; }
...
static ClanIzraza *sab_oper(ClanIzraza *op1, ClanIzraza *op2) {
  int vrednost1 = (op1->klasifikator == SIMBOL ? op1->deo.simbol->vrednost
                                               : op1->deo.literal);
  int vrednost2 = (op2->klasifikator == SIMBOL ? op2->deo.simbol->vrednost
                                               : op2->deo.literal);
...
static SekcijaRel *greska_relokatibilnost(SekcijaRel *sekcija_rel,
                                          Sekcija *sekcija) {
...
void prebaci_postfix(Izraz *izraz) {
  ...
  for (; trenutni; trenutni = trenutni->sledeci) {
    if (trenutni->klasifikator != OPERATOR) {
      ...
      while (!prazan_stek(vrh) && trenutni->deo.operator->irp() <=
                                      top_stek(vrh)->deo.operator->srp()) {
        ClanIzraza *sa_vrha = top_stek(vrh);
        vrh = pop_stek(vrh);
        dodaj_clan(izraz, sa_vrha);
      }
      if (trenutni->klasifikator == OPERATOR &&
          trenutni->deo.operator== & optvf[ZATVORENA_ZAGRADA]) {
        vrh = pop_stek(vrh);
      } else {
...
Simbol *dohvati_referisani_simbol(Izraz *izraz) {
  for (ClanIzraza *trenutni = izraz->prvi; trenutni;
       trenutni = trenutni->sledeci) {
    if (trenutni->klasifikator == SIMBOL &&
        trenutni->deo.simbol->sekcija->simbol->redosled == 0) {
...
Simbol *proveri_relokatibilnost_init_simbol(Izraz *izraz, Sekcija *sekcija,
                                            const char *naziv_simbola) {
  Sekcija *sekcija_rel;
  enum Relokatibilnost status = proveri_relokatibilnost(izraz, &sekcija_rel);

  if (status == NEIZRACUNJIV) {
    NeizracunjiviSimbol *novi =
        init_neizracunjivi_simbol(naziv_simbola, sekcija, izraz);
    dodaj_neizracunjivi_simbol(
        dohvati_asembler()->tabela_neizrazunljivih_simbola, novi);
    return novi->simbol;
  } else if (status == APSOLUTAN) {
    return init_simbolicka_konstanta(naziv_simbola, izraz, sekcija);
  } else if (status == RELOKATIVAN) {
    Simbol *ret = init_definisan_simbol(
        naziv_simbola, izracunaj_vrednost_izraza(izraz), sekcija_rel);
...
enum Relokatibilnost proveri_relokatibilnost(Izraz *izraz, Sekcija **sekcija) {
  ...
  for (ClanIzraza *trenutni = izraz->prvi; trenutni;
    ...
    if (trenutni->klasifikator == OPERATOR) {
      ...
      if (trenutni->deo.operator== & optvf[MNOZENJE] ||
          trenutni->deo.operator== & optvf[DELJENJE]) {
        if (op1->klasifikator == SIMBOL &&
                op1->deo.simbol->tvf->dohvati_relokatibilnost(
                    op1->deo.simbol, &relokativan) == RELOKATIVAN ||
            vrh->clan->klasifikator == SIMBOL &&
                vrh->clan->deo.simbol->tvf->dohvati_relokatibilnost(
                    vrh->clan->deo.simbol, &relokativan) == RELOKATIVAN) {
      ...
      enum Relokatibilnost status =
          op1->deo.simbol->tvf->dohvati_relokatibilnost(op1->deo.simbol,
                                                        &relokativan);

      if (status == NEIZRACUNJIV) {
        return NEIZRACUNJIV;
      } else if (status == RELOKATIVAN) {
        sekcija_rel = trenutni->deo.operator->dodaj_relokatibilnost(
            sekcija_rel, relokativan);
      }
    } else {
      vrh = push_stek(vrh, trenutni);
    }
  ...
  while (!prazan_stek(vrh)) {
    ...
    Sekcija *relokativan;
    enum Relokatibilnost status = op1->deo.simbol->tvf->dohvati_relokatibilnost(
        op1->deo.simbol, &relokativan);
  ...
  if (sekcija_rel == NULL) {
    return APSOLUTAN;
  } else if (sekcija_rel->sledeci == NULL &&
             sekcija_rel->broj_pojavljivanja == 1) {
