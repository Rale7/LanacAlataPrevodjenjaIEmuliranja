...
static void simkonst_ldreg (Simbol* simbol, enum Registar r1, enum Registar r2, Sekcija* sekcija) {
...
static void simkonst_streg (Simbol* simbol, enum Registar r1, enum Registar r2, Sekcija* sekcija) {

  st_reg_literal(r1, r2, simbol->vrednost);
} 

static RelokacioniZapis* simkonst_napr_rel_zapis (Simbol* simbol, Sekcija* sekcija, int lokacija, int obracanje) {
...
static enum Relokatibilnost simkonst_relokitibilnost(Simbol* simbol, Sekcija** sekcija) {
  return APSOLUTAN;
}
...
void razresavanje_neizracunjivog_simbola_konstanta(Simbol* simbol) {
  ...
  for (ObracanjeInstrukcije* obracanje = simbol->oilista; obracanje; obracanje = obracanje->sledeci) {
    ...
    postavi_sadrzaj(obracanje->sekcija->sadrzaj, obracanje->lokacija + 2, &reg_pom, 1);

    char pom = (char) (pomeraj & 0xFF);
    postavi_sadrzaj(obracanje->sekcija->sadrzaj, obracanje->lokacija + 3, &pom, 1);
  }
}