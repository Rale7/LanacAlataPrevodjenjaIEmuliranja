...
static NeizracunjiviElem* init_NE(NeizracunjiviSimbol* ns) {
  NeizracunjiviElem* novi =
      (NeizracunjiviElem*)malloc(sizeof(NeizracunjiviElem));
  if (novi == NULL) {
    exit(1);
  }
...
static int prazan_stek_NE(StekNE* vrh) { return vrh == NULL; }

int topolosko_sortiranje_util(NeizracunjiviElem** graf,
                              NeizracunjiviSimbol** simboli, int indeks,
                              int* obidjeni, StekNE** stek, int* rek_stek) {
  if (!obidjeni[indeks]) {
    obidjeni[indeks] = 1;
    rek_stek[indeks] = 1;

    for (NeizracunjiviElem* tekuci = graf[indeks]; tekuci;
         tekuci = tekuci->sledeci) {
      if (!obidjeni[tekuci->simbol->neizracunjivi_id] &&
          topolosko_sortiranje_util(graf, simboli,
                                    tekuci->simbol->neizracunjivi_id, obidjeni,
                                    stek, rek_stek)) {
        return 1;
...
void topolosko_sortiranje(NeizracunjiviElem** graf,
                          NeizracunjiviSimbol** simboli, int velicina) {
  StekNE* stek = NULL;
  ...
  for (int i = 0; i < velicina; i++) {
    if (!obidjeni[i]) {
      int status = topolosko_sortiranje_util(graf, simboli, i, obidjeni, &stek,
                                             rec_stek);
      if (status == 1) {
  ...
  while (!prazan_stek_NE(stek)) {
    ...
    enum Relokatibilnost status =
        proveri_relokatibilnost(ns->izraz, &sekcija_rel);

    if (status == APSOLUTAN) {
      prebaci_u_simbolicku_konstantu(ns->simbol,
                                     izracunaj_vrednost_izraza(ns->izraz));
      razresavanje_neizracunjivog_simbola_konstanta(ns->simbol);
    } else if (status == RELOKATIVAN) {
      prebaci_u_definisan(ns->simbol, sekcija_rel,
                          izracunaj_vrednost_izraza(ns->izraz));
...
void razresi_TNS(TNS* tns) {
  NeizracunjiviElem** graf = (NeizracunjiviElem**)calloc(
      tns->broj_simbola, sizeof(NeizracunjiviElem*));
  NeizracunjiviSimbol** simboli = (NeizracunjiviSimbol**)calloc(
      tns->broj_simbola, sizeof(NeizracunjiviSimbol));
  if (graf == NULL) {
    exit(1);
  }

  int cnt = 0;
  for (NeizracunjiviElem* tekuci = tns->prvi; tekuci;
       tekuci = tekuci->sledeci) {
    simboli[tns->broj_simbola - ++cnt] = tekuci->simbol;

    for (ClanIzraza* clan = tekuci->simbol->izraz->prvi; clan;
         clan = clan->sledeci) {
      int indeks;
