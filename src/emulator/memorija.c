...
static int dohvati_vrednost_memorija(Segment* segment, unsigned int adresa) {
  int* adresa_sadrzaja =
      (int*)(segment->sadrzaj + adresa - segment->pocetna_adresa);

  return *adresa_sadrzaja;
}

static void postavi_vrednost_memorija(Segment* segment, unsigned int adresa,
                                      int vrednost) {
...
Segment* init_segment_sadrzaj(unsigned int pocetna_adresa,
                              unsigned int krajnja_adresa, char* sadrzaj) {
...
Segment* init_segment(unsigned int pocetna_adresa,
...
static Segment* pretrazi(Memorija* memorija, unsigned int adresa) {
  ...
  if (prethodnik && sledbenik) {
    if (sledbenik->pocetna_adresa - prethodnik->krajnja_adresa <
        DEFAULT_SEGMENT_SIZE) {
      ubaci_segment(memorija,
                    (novi = init_segment(prethodnik->krajnja_adresa + 1,
                                         sledbenik->pocetna_adresa - 1)));
    } else {
      ubaci_segment(memorija, (novi = init_segment(
                                   max(prethodnik->krajnja_adresa + 1,
                                       adresa - DEFAULT_SEGMENT_SIZE / 2),
                                   min(sledbenik->pocetna_adresa - 1,
                                       adresa + DEFAULT_SEGMENT_SIZE / 2))));
    }
  } else if (sledbenik) {
    if (sledbenik->pocetna_adresa < DEFAULT_SEGMENT_SIZE) {
      ubaci_segment(memorija,
                    (novi = init_segment(0, sledbenik->pocetna_adresa - 1)));
    } else {
      ubaci_segment(memorija, novi = init_segment(
    ...
    if (0xFFFFFEFFU - prethodnik->pocetna_adresa < DEFAULT_SEGMENT_SIZE) {
      ubaci_segment(
          memorija,
          (novi = init_segment(prethodnik->pocetna_adresa + 1, 0xFFFFFEFFU)));
    } else {
      ubaci_segment(memorija,
                    novi = init_segment(
                        max(prethodnik->krajnja_adresa + 1,
    ...
    ubaci_segment(memorija,
                  novi = init_segment(adresa - DEFAULT_SEGMENT_SIZE / 2,
                                      adresa + DEFAULT_SEGMENT_SIZE / 2));
  }
...
void inorder_memorija(Segment* segment) {
  if (segment != NULL) {
    ...
    printf("Segment pocetna-%x, kranja-%x\n", segment->pocetna_adresa,
           segment->krajnja_adresa);
