...
static inline void dohvati_registre_pomeraj(char instrukcija[4], char* ra, char* rb, char* rc, int* disp) {
  ...
  if (disp) {
    *disp = ((((int)instrukcija[2]) & 0x0F)<< 8) | ((int)instrukcija[3] & 0xFF);
    if (*disp & 0x800) {
...
static int bgt_indirect(Procesor* procesor, Memorija* memorija, char instrukcija[4]) {
...
static int sabiranje_op(int a, int b) {
  return a + b;
}

static int sabiranje(Procesor* procesor, Memorija* memorija, char instrukcija[4]) {
  return operacije(procesor, instrukcija, &sabiranje_op);
}

static int oduzimanje_op(int a, int b) {
  return a - b;
}

static int oduzimanje(Procesor* procesor, Memorija* memorija, char instrukcija[4]) {
  return operacije(procesor, instrukcija, &oduzimanje_op);
}

static int mnozenje_op(int a, int b) {
  return a * b;
}

static int mnozenje(Procesor* procesor, Memorija* memorija, char instruckija[4]) {
  return operacije(procesor, instruckija, &mnozenje_op);
}

static int deljenje_op(int a, int b) {
  return a / b;
}

static int deljenje(Procesor* procesor, Memorija* memorija, char instrukcija[4]) {
  return operacije(procesor, instrukcija, &deljenje_op);
}

static int logicko_ne(Procesor* procesor, Memorija* memorija, char instrukcija[4]) {
...
static int iop(int a, int b) {
  return a & b;
}

static int logicko_i(Procesor* procesor, Memorija* memorija, char instrukcija[4]) {
  return operacije(procesor, instrukcija, &iop);
}

static int iliop(int a, int b) {
  return a | b;
}

static int logicko_ili(Procesor* procesor, Memorija* memorija, char instrukcija[4]) {
...
static int logicko_xor(Procesor* procesor, Memorija* memroia, char instrukcija[4]) {
...
static int pomeranje_ulevo(Procesor* procesor, Memorija* memorija, char instrukcija[4]) {
...
static int pomeranje_udesno(Procesor* procesor, Memorija* memorija, char instrukcija[4]) {
  return operacije(procesor, instrukcija, &shr);
}

static int st_mem_dir(Procesor* procesor, Memorija* memorija, char instrukcija[4]) {
...
static int st_mem_ind(Procesor* procesor, Memorija* memorija, char instrukcija[4]) {
...
static int st_regpom(Procesor* procesor, Memorija* memorija, char instrukcija[4]) {
...
static int mov_csr(Procesor* procesor, Memorija* memorija, char instrukcija[4]) {
  char ra, csr;
...
static int mov_disp(Procesor* procesor, Memorija* memorija, char instrukcija[4]) {
  char ra, rb;
  int disp;
...
static int ld_mem_disp(Procesor* procesor, Memorija* memorija, char instrukcija[4]) {
  char ra, rb;
  int disp;
...
static int ld_csr_mask(Procesor* procesor, Memorija* memorija, char instrukcija[4]) {
  char csr, rb;
  int disp;
...
static int ld_csr_mem(Procesor* procesor, Memorija* memorija, char instrukcija[4]) {
  char csr, rb, rc;
  int disp;
...
static int ld_csr_mem_disp(Procesor* procesor, Memorija* memorija, char instrukcija[4]) {
  char csr, rb;
  int disp;
