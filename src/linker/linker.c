...
static inline Elf32_Shdr* safe_realloc(Elf32_Shdr* shrd, int broj_zagljavlja) {
  ...
  if (broj_zagljavlja > 0 && broj_zagljavlja % 10 == 0) {
    ...
    shrd = (Elf32_Shdr*) realloc(shrd, (broj_zagljavlja + 10) * sizeof(Elf32_Shdr));
    if (shrd == NULL) {
      exit(1);
    }
  }
...
void napravi_relokativni_fajl(Linker* linker, const char* ime_izlaznog_fajla) {
  ...
  zagljavlja[broj_zaglavlja++] = (Elf32_Shdr) {
    .sh_name = dodaj_string(shstr, "UND"),
    .sh_type = SHT_NULL,
    .sh_flags = 0,
    .sh_addr = 0,
    .sh_offset = 0,
  ...
  for (int i = 0; i < linker->tabela_sekcija->broj_sekcija; i++) {
    ...
    if (sekcija->prvi) {
      indeks_stringa_rel = dodaj_string_povezano(shstr, "rel", sekcija->simbol.naziv);
    ...
    zagljavlja[broj_zaglavlja++] = (Elf32_Shdr) {
      .sh_name = indeks_stringa_sekcija,
  ...
  for (int i = 0; i < linker->tabela_simbola->trenutni_id; i++) {
    ...
    Elf32_Sym novi_simbol = {
      ...
      .st_shndx = (simbol->sekcija ? simbol->sekcija->broj_elf_ulaza : simbol->tvf->dohvati_sekciju(simbol)),
  ...
  zagljavlja[broj_zaglavlja++] = (Elf32_Shdr) {
    .sh_name = dodaj_string(shstr, "symtab"),
    .sh_type = SHT_SYMTAB,
    .sh_flags = 0,
    .sh_addr = 0,
