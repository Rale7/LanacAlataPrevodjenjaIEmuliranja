...
static RelokacioniZapis* sekcija_relokacioni_zapis(Simbol* simbol, int offset,
                                                   int addend) {
...
static char dohvati_bind_sekcije(Simbol* simbol) { return STB_LOCAL; }

static char dohvati_tip_sekcije(Simbol* simbol) { return STT_SECTION; }
...
void dodaj_relokacioni_zapis(Sekcija* sekcija,
...
void razresi_relokacije(Sekcija* sekcija) {
  for (RelokacioniZapis* tekuci = sekcija->prvi; tekuci;
       tekuci = tekuci->sledeci) {
    int zamena =
        tekuci->simbol->tvf->dohvati_vrednost(tekuci->simbol) + tekuci->addend;
